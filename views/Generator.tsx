...
import { 
  ...
  Image as ImageIcon, Upload, Camera, Tag, X, FileImage
} from 'lucide-react';
...
const Generator: React.FC<GeneratorProps> = ({ onSuccess, usage, onDeleteImage, onDownloadImage }) => {
  ...
  const AssetUploader = ({ label, type, icon: Icon, value }: { label: string, type: keyof AssetUploads, icon: any, value: string | null }) => (
    <div className="space-y-3">
      ...
      <div className="relative">
        {value ? (
          <div className="relative group aspect-square rounded-2xl overflow-hidden border-2 border-indigo-600 shadow-lg">
          ...
          <label className="flex flex-col items-center justify-center aspect-square border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group">
            ...
            <span className="text-[9px] font-black text-gray-400 uppercase tracking-tighter mt-2">Upload</span>
            <input 
              type="file" 
              accept="image/*" 
              className="hidden" 
        ...
        <div className="bg-white border-2 border-indigo-50 p-10 md:p-14 rounded-[3.5rem] text-center space-y-10 shadow-2xl">
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-500 font-bold leading-relaxed max-w-xs mx-auto">
              Conecte seu motor criativo para começar a renderizar campanhas de alto nível.
            </p>
          </div>
          <button 
          ...
          >
            <ShieldCheck size={20} className="text-indigo-400" /> Ativar Estúdio
          </button>
        </div>
      </div>
    );
        ...
        {generatedResult ? (
          <div className="animate-in zoom-in-95 duration-500 space-y-8">
              ...
              {generatedResult.sources.length > 0 && (
                <div className="bg-indigo-50/50 p-6 rounded-[1.5rem] border border-indigo-100">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[10px] font-black uppercase text-indigo-600 tracking-widest">Contexto de Mercado</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {generatedResult.sources.map((src, i) => src.web && (
                      <a key={i} href={src.web.uri} target="_blank" className="bg-white px-3 py-1.5 rounded-full text-[9px] font-bold text-gray-500 hover:text-indigo-600 border border-indigo-100 flex items-center gap-1.5 transition-colors">
                        <LinkIcon size={10} /> {src.web.title || 'Link'}
                      </a>
                    ))}
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 gap-4">
               <button onClick={() => setGeneratedResult(null)} className="py-5 bg-gray-100 text-gray-900 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-gray-200 transition-colors">Nova Campanha</button>
               <button onClick={() => onDownloadImage(generatedResult.url, 'campanha.png')} className="py-5 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-700 shadow-lg flex items-center justify-center gap-2">
          ...
          <div className="space-y-12 animate-in fade-in duration-700">
            {/* Step 1: Niche */}
            <div className="space-y-4">
              <label className="text-xs font-black text-gray-400 uppercase tracking-widest">1. Selecione o Nicho</label>
            ...
            {/* Step 2: Assets */}
            <div className="space-y-4">
              <label className="text-xs font-black text-gray-400 uppercase tracking-widest">2. Ativos da Marca (Upload)</label>
              <div className="grid grid-cols-3 gap-4">
                <AssetUploader label="Foto do Produto" type="productPhoto" icon={Camera} value={assets.productPhoto} />
                <AssetUploader label="Logo da Marca" type="brandLogo" icon={Tag} value={assets.brandLogo} />
                <AssetUploader label="Referência Estilo" type="styleReference" icon={FileImage} value={assets.styleReference} />
              </div>
              <p className="text-[9px] text-gray-400 font-medium italic">Dica: Adicione uma referência para a IA entender a iluminação desejada.</p>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                ...
                <textarea 
                  placeholder="Descreva detalhes como: 'Luz de neon', 'Vapor saindo', 'Fundo minimalista'..." 
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-4">
                   <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Valor do Produto</label>
                ...
                <div className="space-y-4">
                   ...
                   <select className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl px-6 py-4 font-bold text-[10px] uppercase" value={config.aspectRatio} onChange={(e) => setConfig({...config, aspectRatio: e.target.value as any})}>
                     <option value="1:1">Post Quadrado (1:1)</option>
            ...
            <button 
              ...
              disabled={isGenerating || !assets.productPhoto} 
              className={`w-full py-7 rounded-[2.5rem] font-black text-xl shadow-2xl transition-all flex items-center justify-center gap-4 active:scale-[0.98] ${
                !assets.productPhoto ? 'bg-gray-100 text-gray-300 cursor-not-allowed' : 'bg-gray-950 text-white hover:bg-indigo-600'
              }`}
            >
              <Sparkles size={28} className={assets.productPhoto ? "text-indigo-400" : "text-gray-300"} /> 
              {assets.productPhoto ? 'RENDERIZAR CAMPANHA' : 'ENVIE A FOTO DO PRODUTO'}
            </button>
            ...
            <button onClick={() => setNeedsKeySelection(true)} className="w-full flex items-center justify-center gap-2 text-[9px] font-black text-gray-300 uppercase tracking-widest hover:text-indigo-400 transition-colors">
              <RefreshCw size={10} /> Resetar Conexão IA
            </button>
          </div>
        ) : (
          <div className="py-24 text-center space-y-10 animate-in fade-in duration-500">
            ...
            <div className="space-y-3">
              ...
              <p className="text-[10px] text-gray-400 font-black uppercase tracking-[0.4em]">Criando arte com Inteligência de Elite...</p>
            </div>
          </div>
        )}
      </div>
